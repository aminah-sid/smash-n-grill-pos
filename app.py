...
<body>
    <h2>Smash and Grill</h2>
    <div class="small">Order ID: {order_id} &nbsp;|&nbsp; Date/Time: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</div>
    ...
    <table class="summary" style="width: 100%; margin-top: 10px;">
        <tr><td class="right">Subtotal</td><td class="right" style="width:100px">{subtotal:.0f}</td></tr>
...
def add_to_cart(row, qty):
    # If item exists, increase qty; else append
...
def main():
    ...
    if item_name:
        row = filtered[filtered["Item"]==item_name].iloc[0]
    else:
        row = None
    ...
    if add_col.button("➕ Add to Cart", use_container_width=False, disabled=(row is None)):
        if row is not None:
            row = row.copy()
    ...
    checkout_btn = st.button("✅ Checkout & Save Order", type="primary", disabled=df_cart.empty)
    if checkout_btn:
        if df_cart.empty:
